...
from models.fcdgcn import fcdgcnClassifier
from models.fcdgcn_bert import fcdgcnBertClassifier
...
class Instructor:
    ...
    def _print_args(self):
        ...
        for p in self.model.parameters():
            n_params = p.numel()  # 或 int(torch.tensor(p.shape, dtype=torch.long).prod().item())
            if p.requires_grad:
    ...
    def _reset_params(self):
        for name, p in self.model.named_parameters():
            if not p.requires_grad:
                continue
            if name.endswith('gate_pre') or name.endswith('gate_post_a') or name.endswith('gate_post_b') \
            or 'gate_inloop' in name:
                continue

            if p.dim() >= 2:
                self.opt.initializer(p)   # e.g. xavier_uniform_
            else:
                if 'bias' in name:
                    nn.init.zeros_(p)
                else:
                    size0 = p.numel() if p.dim() == 0 else p.shape[0]
                    stdv = 1.0 / (size0 ** 0.5)
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                for col in self.opt.inputs_cols:
                    t = sample_batched[col]
                    if col == 'length':
        # pack_padded_sequence 要求 CPU int64
                        inputs.append(t.detach().cpu().long().view(-1))
                    else:
                        inputs.append(t.to(self.opt.device))
...
def main():
    model_classes = {
        'fcdgcn': fcdgcnClassifier,
        'fcdgcnbert': fcdgcnBertClassifier,
    ...
    input_colses = {
        ...
        'fcdgcn': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length','short_mask'],
        'fcdgcnbert': ['text_bert_indices', 'bert_segments_ids', 'attention_mask', 'asp_start', 'asp_end', 'src_mask', 'aspect_mask','short_mask']
    }
    ...
    parser.add_argument('--model_name', default='fcdgcn', type=str, help=', '.join(model_classes.keys()))
